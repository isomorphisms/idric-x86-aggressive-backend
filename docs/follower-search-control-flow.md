...
This is the preserved PR #17 lower-level control-flow regression. The checked
scalar compiler handoff now exists in `checked-x86-baseline.md`; strings and
indexed byte loads are not yet in that deliberately small compiler-facing
surface.
...
The remaining compiler-facing task for this slice is to extend the checked
one-step-at-a-time handoff with integer loops and byte loads without importing
Thumb-specific representation choices. Until then this target-side slice
stays an honest backend fixture.
