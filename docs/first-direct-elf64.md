# Checked mathematical direct ELF64 path
...
checked artifact.  `backend/idric_x86.py` does not accept Idriç source syntax,
...
Run the complete handoff against a checked-out Idriç compiler with:

```sh
make integration IDRIC_REPO=/path/to/Idric
```

That target invokes exactly
`edric --emit-math-one-step SOURCE -o ARTIFACT`, verifies the artifact against
the same source bytes, and only then compiles and runs it.  The environment-
controlled integration unittest reports `SKIP` when the real emitter/source are
not configured; it never substitutes the test double into the claimed
front-end path.
